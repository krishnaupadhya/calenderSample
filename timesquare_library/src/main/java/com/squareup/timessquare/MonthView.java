...
public class MonthView extends LinearLayout {
  ...
  public static MonthView create(ViewGroup parent, LayoutInflater inflater,
    ...
    view.setGridTypeface();
    view.setTitleTypeface();
    ...
    for (int offset = 0; offset < 7; offset++) {
      ...
      if(weekdayNameFormat.format(today.getTime()) != null && !TextUtils.isEmpty(weekdayNameFormat.format(today.getTime()))) {
        String weekday;
        if(locale.equals(Locale.ENGLISH)) {
          weekday = weekdayNameFormat.format(today.getTime()).charAt(0) + "";
        } else {
          weekday = weekdayNameFormat.format(today.getTime());
        }
  ...
  public void setImmediateDateVisibility(int visibility) {
  ...
  private void setGridTypeface() {
    if (grid == null)
      return;

      grid.setTypeface(Typeface.create("sans-serif", Typeface.NORMAL));
  }
