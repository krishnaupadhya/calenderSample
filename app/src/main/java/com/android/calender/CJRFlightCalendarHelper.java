 ...
 */
public class CJRFlightCalendarHelper implements CalendarPickerView.OnDateSelectedListener, View.OnClickListener,Observer {
    ...
    private Calendar nextYear,currentYear;
    ...
    public void initView(View view, int position , boolean useLoaderAdapter) {
        ...
        priceInfo = CJRFareCalendarDataTransformer.getObserver().getCalendarPriceModel();
    ...
    private void setSingleCalenderUI(String mSelectedCheckInDate) {
        Date selectedCheckInDate = CJRHotelCalenderUtils.returnDateValue(mActivity, mSelectedCheckInDate);
    ...
    private void setRangeCalenderUI() {
        ...
        if(mSelectedCheckInDate != null) {
            Date selectedCheckInDate = CJRHotelCalenderUtils.returnDateValue(mActivity, mSelectedCheckInDate);
            Date selectedCheckOutDate = CJRHotelCalenderUtils.returnDateValue(mActivity, mSelectedCheckOutDate);
...
//            }
    ...
    public void onClick(View v) {
        if( v == mTodaysDateSelector) {
            ...
            if(mIntentType != null && mIntentType.equalsIgnoreCase(Constants.INTENT_EXTRA_SELECTED_DEPART_DATE)) {
...
//        }
    ...
    private void returnToFlightsHome(String date, boolean isFromTodayBtn) {
        if (mIntentType != null && mIntentType.equalsIgnoreCase(Constants.INTENT_EXTRA_SELECTED_DEPART_DATE)) {
            ...
            if (CJRHotelCalenderUtils.compareIfLastAvailableCheckInDate(mActivity, date, appLocale)) {
                return;
            } else {
            ...
            if (mIntentType != null) {
                ...
                if (mIntentType.equalsIgnoreCase(Constants.INTENT_EXTRA_SELECTED_DEPART_DATE)) {
                    mResultIntent.putExtra(Constants.INTENT_EXTRA_SELECTED_DEPART_DATE, date);
                } else {
...
//                        CJRAppUtility.showAlert(mActivity, mActivity.getResources().getString(R.string.error), mActivity.getResources().getString(R.string.select_check_in_date_alert_msg));
                    ...
                    if (compareTwoDates(date, mOtherDate, mActivity.getResources().getString(R.string.flight_return))) {
                        setCalenderUI();
                        return;
                    }

                    if (mSelectedCheckInDate != null) {
                        mResultIntent.putExtra(Constants.INTENT_EXTRA_SELECTED_DEPART_DATE, mSelectedCheckInDate);
                    }
                    mResultIntent.putExtra(Constants.INTENT_EXTRA_SELECTED_RETURN_DATE, date);
                }
            }
    ...
    private boolean compareTwoDates(String inFormattedDate, String inOtherDate, String inDateType) {
        try {
            ...
            if(isScrolledToCheckOut && mSelectedCheckInDate != null) {
                ...
                if(!inOtherDate.equalsIgnoreCase(mActivity.getResources().getString(R.string.flight_return))) {
            ...
            if(inDateType.equalsIgnoreCase(mActivity.getResources().getString(R.string.flight_return)) ) {
    ...
    private void updateCalenderUI(String date) {
        ...
        if(mSelectedCheckInDate != null) {
            Date selectedCheckInDate = CJRHotelCalenderUtils.returnDateValue(mActivity, mSelectedCheckInDate);
            Date selectedCheckOutDate = CJRHotelCalenderUtils.returnDateValue(mActivity, date);
            ...
            if(mDatePriceMap == null  || mDatePriceMap.size() == 0) {
                if(mUseLoaderAdapter) {
                    calendar.setCustomDayView(new FlightLoaderDayViewAdapter());
                } else {
                    calendar.setCustomDayView(new DefaultDayViewAdapter());
                }
//            List<CalendarCellDecorator> decoratorList = new ArrayList<>();
//            decoratorList.add(new CJRFlightFareProgressDecorator(this, null, mDateToBeSelected));
//            calendar.setDecorators(decoratorList);
            } else {
                calendar.setCustomDayView(new FlightDayViewAdapter());
//            calendar.setCustomDayView(new DefaultDayViewAdapter());

            }

            List<CalendarCellDecorator> decoratorList = new ArrayList<>();
            decoratorList.add(new FlightCalendarCellDecorator(mActivity.getApplicationContext() , mDatePriceMap  , true , new Handler()));
            calendar.setDecorators(decoratorList);
    ...
    private void setCheckInFooterViews(String isFrom, String mCheckInDatePreviouslySelected) {
        try {
            ...
            if(mCheckInDatePreviouslySelected != null && !mCheckInDatePreviouslySelected.equalsIgnoreCase("Check-in Date")
                ...
                mTodayDate.setText(AppUtility.calenderFormatDate(mActivity, mOneNightDate, Constants.HOTELS_DISPLAY_DATE_FORMAT, Constants.HOTELS_HOMESCREEN_DISPLAY_DATE));
                mTommorrowDate.setText(AppUtility.calenderFormatDate(mActivity, mTwoNightDate, Constants.HOTELS_DISPLAY_DATE_FORMAT, Constants.HOTELS_HOMESCREEN_DISPLAY_DATE));
            ...
            // else normally it is tomorrow and day after tomorrow dates
                ...
                if (isFrom != null && isFrom.equalsIgnoreCase("CheckIN")) {
                    mTodayDate.setText(AppUtility.calenderFormatDate(mActivity, df.format(mTodayCalender.getTime()), Constants.HOTELS_DISPLAY_DATE_FORMAT, Constants.HOTELS_HOMESCREEN_DISPLAY_DATE));
                    mTommorrowDate.setText(AppUtility.calenderFormatDate(mActivity, mTomDate, Constants.HOTELS_DISPLAY_DATE_FORMAT, Constants.HOTELS_HOMESCREEN_DISPLAY_DATE));
                }
                ...
                if (isFrom != null && isFrom.equalsIgnoreCase("CheckOUT")) {
                    mTodayDate.setText(AppUtility.calenderFormatDate(mActivity, mTomDate, Constants.HOTELS_DISPLAY_DATE_FORMAT, Constants.HOTELS_HOMESCREEN_DISPLAY_DATE));
                    mTommorrowDate.setText(AppUtility.calenderFormatDate(mActivity, mDayAfterTomDate, Constants.HOTELS_DISPLAY_DATE_FORMAT, Constants.HOTELS_HOMESCREEN_DISPLAY_DATE));
                }
            }
...
//            calendar.getDecorators().clear();
        ...
        List<CalendarCellDecorator> decoratorList = new ArrayList<>();
        decoratorList.add(new FlightCalendarCellDecorator(mActivity , mDatePriceMap  , shouldStopLoader , new Handler()));
        calendar.setDecorators(decoratorList);
