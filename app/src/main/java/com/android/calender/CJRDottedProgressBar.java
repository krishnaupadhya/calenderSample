...
/**
 * Created by Abhishek V on 30/03/2016.
 */
public class CJRDottedProgressBar extends LinearLayout {
    ...
    public CJRDottedProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CJRDottedProgressBar);
        if (typedArray != null) {
            int size = typedArray.getInteger(R.styleable.CJRDottedProgressBar_dot_size, -1);
            if (size != -1) {
                mDotSize = size;
            }

            int dotMargin = typedArray.getInteger(R.styleable.CJRDottedProgressBar_dot_margin, -1);
            if (dotMargin != -1) {
    ...
    public CJRDottedProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initViews();
    }

    public CJRDottedProgressBar(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
